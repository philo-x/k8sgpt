...
func initConfig() {
	...
	// Register the default cluster in the global ClusterManager so that all
	// subsystems (MCP server, analysis engine, …) can look it up by ID.
	// We do a best-effort registration: if the cluster is unreachable at startup
	// (e.g. no kubeconfig present) we silently skip; each subsystem has its own
	// fallback and will produce a proper error when actually invoked.
	_, _ = kubernetes.DefaultManager().RegisterCluster(
		kubernetes.DefaultClusterID,
		kubeconfig,
		kubecontext,
	)
}
