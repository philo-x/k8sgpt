...
func init() {
	...
	// name flag
	AnalyzeCmd.Flags().StringVar(&name, "name", "", "Name of the Kubernetes resource to analyze")
	// print stats
	AnalyzeCmd.Flags().BoolVarP(&withStats, "with-stat", "s", false, "Print analysis stats. This option disables errors display.")
	// cluster flag
	AnalyzeCmd.Flags().StringVar(&clusterID, "cluster", "", "Target cluster ID as registered in k8sgpt (empty means default cluster)")
}
