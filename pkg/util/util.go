...
// GetCacheKeyWithCluster is like GetCacheKey but also incorporates the cluster
// identifier so that results from different clusters never share a cache entry.
func GetCacheKeyWithCluster(provider, language, clusterID, sEnc string) string {
	data := fmt.Sprintf("%s-%s-%s-%s", provider, language, clusterID, sEnc)
