...
type Analysis struct {
	...
	// ClusterID identifies which cluster this analysis targets.
	// An empty string or kubernetes.DefaultClusterID means the default cluster.
	ClusterID string
}
...
func NewAnalysis(
	...
	httpHeaders []string,
	withStats bool,
	// clusterID identifies which registered cluster to analyse.
	// Pass an empty string (or kubernetes.DefaultClusterID) to use the default cluster.
	clusterID string,
) (*Analysis, error) {
	...
	// Resolve the Kubernetes client via ClusterManager when a clusterID is given,
	// otherwise fall back to the legacy viper-based approach so that CLI usage
	// (which registers the "default" cluster via root.go) continues to work.
	var client *kubernetes.Client
	var err error
	mgr := kubernetes.DefaultManager()
	if clusterID == "" {
		clusterID = kubernetes.DefaultClusterID
	}
	if mgr.HasCluster(clusterID) {
		client, err = mgr.GetClientOrDefault(clusterID)
		if err != nil {
			return nil, fmt.Errorf("initialising kubernetes client for cluster %q: %w", clusterID, err)
		}
	} else {
		// Fallback: create a one-off client from viper config (backwards-compat).
	...
	if verbose {
		fmt.Printf("Debug: Kubernetes client initialized, server=%s, cluster=%s.\n", client.Config.Host, clusterID)
	}
...
func (a *Analysis) RunAnalysis() {
	defer func() {
		// Filter results by Name if specified
		if a.Name != "" {
			var filteredResults []common.Result
			for _, result := range a.Results {
				if result.Name == a.Name || strings.HasSuffix(result.Name, "/"+a.Name) {
					filteredResults = append(filteredResults, result)
				}
			}
			a.Results = filteredResults
		}
	}()
...
func (a *Analysis) getAIResultForSanitizedFailures(texts []string, promptTmpl string) (string, error) {
	...
	// Use cluster-aware cache key so that identical errors from different clusters
	// do not share the same cached AI response.
	cacheKey := util.GetCacheKeyWithCluster(a.AIClient.GetName(), a.Language, a.ClusterID, inputKey)
