...
func NewMCPServer(port string, aiProvider *ai.AIProvider, useHTTP bool, logger *zap.Logger) (*K8sGptMCPServer, error) {
	...
	var k8sGptMCPServer = &K8sGptMCPServer{
		...
		useHTTP:        useHTTP,
		logger:         logger,
		clusterManager: kubernetes.DefaultManager(),
	}
...
func (s *K8sGptMCPServer) registerToolsAndResources() error {
	...
	analyzeTool := mcp.NewTool("analyze",
		...
		mcp.WithString("name",
			mcp.Description("Name of the specific Kubernetes resource to analyze (optional)"),
		),
		...
		mcp.WithString("cluster",
			mcp.Description("Target cluster ID (use list-clusters to see available IDs; empty means default cluster)"),
		),
	)
	...
	// Register list-clusters tool — lets LLMs discover which clusters are available.
	listClustersTool := mcp.NewTool("list-clusters",
		mcp.WithDescription("List all Kubernetes clusters registered with k8sgpt. Use the returned IDs in the 'cluster' parameter of other tools."),
	)
	s.server.AddTool(listClustersTool, s.handleListClusters)
...
type AnalyzeRequest struct {
	...
	// Cluster identifies the target cluster. Use list-clusters to discover available IDs.
	// An empty value selects the default cluster.
...
func (s *K8sGptMCPServer) handleAnalyze(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	if s.aiProvider.Name != "" {
		backend = s.aiProvider.Name
	} else {
		backend = "openai" // fallback default
	}
	explain := false
...
func (s *K8sGptMCPServer) handleClusterInfo(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	...
	// Resolve client via ClusterManager
	client, err := s.clusterManager.GetClientOrDefault(req.Cluster)
	if err != nil {
		return mcp.NewToolResultErrorf("failed to get Kubernetes client for cluster %q: %v", req.Cluster, err), nil
	}
