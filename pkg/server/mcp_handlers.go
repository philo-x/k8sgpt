...
// handleListClusters returns all Kubernetes clusters registered with k8sgpt.
// LLMs should call this tool first to discover available cluster IDs, then pass
// those IDs in the 'cluster' parameter of other tools.
func (s *K8sGptMCPServer) handleListClusters(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	infos := s.clusterManager.ListClusters()
	...
	summaries := make([]clusterSummary, 0, len(infos))
	...
	result := map[string]interface{}{
		"clusters": summaries,
		"count":    len(summaries),
		"note":     "Use the 'id' field as the 'cluster' parameter in other tools. Empty 'cluster' parameter defaults to the '" + kubernetes.DefaultClusterID + "' cluster.",
	}

	resultJSON, err := marshalJSON(result)
	if err != nil {
		return mcp.NewToolResultErrorf("Failed to serialize clusters: %v", err), nil
	}
